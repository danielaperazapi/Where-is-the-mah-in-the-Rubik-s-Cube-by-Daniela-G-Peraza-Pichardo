...
1. The identity element _e_ is unique.

2. If 𝑎∗𝑏 = ⅇ, then 𝑎 = 𝑏⁻¹

3. If 𝑎∗𝑥 = 𝑏∗𝑥, then 𝑎 = 𝑏

4. The inverse of (𝑎𝑏) is 𝑏⁻¹ 𝑎⁻¹
...
In general, we can determine a move sequence that takes the cube from a given starting position to a given final position, but the answer is usually not unique. In other words, there is exactly one permutation that converts the starting configuration into the final configuration. However, the same element can be represented by many different move sequences, for example:
...
One of the main challenges of this project was connecting abstract mathematical definitions with the physical movements of the cube. While the notation and algorithms are relatively easy to learn, understanding how a sequence of moves corresponds to a permutation of cubies required a deeper level of analysis, especially in compositions of permutations. Another challenge was learning how group theory provides explanations rather than simply computational procedures. It was necessary to understand not only how algorithms change the cube, but also how concepts such as commutators and non-commutativity allow specific pieces to be manipulated while preserving others.
...
This source is very useful to understand the practical side of Rubik's Cube solving, particularly the stages involved in common solving methods such as CFOP. It provides accessible explanations of solving algorithms and it also helps contextualize the mathematical concepts discussed in this repository. While the academic sources focused on theory, this reference helped explain how those mathematical ideas are applied during the actual solving process.
...
This source serves as one of the primary references for connecting the Rubik's Cube with abstract algebra. It provided explanations of permutations, cycle notation, and the mathematical structure of cube moves. The presentation was particularly useful for understanding how cube algorithms can be represented as permutations and for illustrating the relationship between physical cube movements and mathematical objects since it contained various illustrations.
